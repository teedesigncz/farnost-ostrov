...
| **Ohlášky** | Upload PDF (nepovinný), správa ohlášek → `ohlasky/ohlasky.json` + `ohlasky/ohlasky-archiv.json`. Auto-archivace podle `datum_archiv`. |
...
```json
{
  ...
  "datum_od": "YYYY-MM-DD",
  "datum_do": "YYYY-MM-DD",
  "datum_zobraz": "YYYY-MM-DD",
  "datum_archiv": "YYYY-MM-DD",
  "pinnováno": false
}
```

- `soubor` – může být prázdný řetězec (příloha je nepovinná)
- `datum_od` / `datum_do` – pouze pro zobrazení na webu (za názvem)
- `datum_zobraz` – ohláška se na homepage nezobrazí dříve než toto datum
- `datum_archiv` – admin při načtení automaticky přesune prošlé záznamy do archivu
- `pinnováno` – připnuté ohlášky jsou řazeny před ostatními
- Homepage zobrazuje vždy jen **první viditelnou** ohlášku ze seznamu
...
Přes admin konzoli (`/admin/` → sekce Ohlášky). Ruční přidání:

1. Nahrát PDF do složky `ohlasky/` (volitelné).
2. Přidat záznam **na začátek** pole v `ohlasky/ohlasky.json` (viz schéma výše).
